...
kpx=map(analogRead(A0),0,1023,20,40);
kdx=map(analogRead(A1),0,1023,-15,0);
kix=map(analogRead(A2),0,1023,0,10);
//
...
//    accelgyro.setZAccelOffset(-15);
  ...
  accX = ((i2cData[0] << 8) | i2cData[1])-(-919);
  accY = ((i2cData[2] << 8) | i2cData[3])- (-2148);
  accZ = ((i2cData[4] << 8) | i2cData[5])- (5544);
...
Serial.println(Ki);
  ...
  runEvery(25)  // run code @ 40 Hz
  {
    ...
    if (CurrentAngle < 245 && CurrentAngle > 120)
    {
    Pid();
    Motors();
    }
...
void dof()
{
  ...
  gyroX = ((i2cData[8] << 8) | i2cData[9]) -(71);
  gyroY = ((i2cData[10] << 8) | i2cData[11]) - (-23);
  gyroZ = ((i2cData[12] << 8) | i2cData[13]) - (-15);
  accXangle = (atan2(accY,accZ)+PI)*RAD_TO_DEG;
